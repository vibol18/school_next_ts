...
export default function SentMessagesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['messages-sent', userId] });
  ...
  const columns: Column<Message>[] = [
    {
      key: 'receiverName',
      header: 'To',
      render: (m) => <span className="font-medium text-[#111827]">{m.receiverName}</span>,
    },
    {
      key: 'subject',
      header: 'Subject',
      render: (m) => (
        <div className="min-w-0">
          <span className="text-[#111827] font-normal">{m.subject}</span>
          {m.body && <p className="text-xs text-[#6b7280] truncate max-w-xs">{m.body}</p>}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: 'Status',
      render: (m) => (
        <Badge variant={m.isRead ? 'green' : 'amber'}>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#111827]">Sent Messages</h1>
        <p className="text-sm text-[#6b7280]">Messages you have sent</p>
      </div>

      <ListToolbar
        searchValue={search}
        onSearchChange={setSearch}
        searchPlaceholder="Search messages by recipient or subject..."
        actionLabel="Compose Message"
        onAction={() => setComposeOpen(true)}
      />

      {isLoading ? (
        <LoadingSpinner text="Loading sent messages..." />
      ) : (
        <DataTable
          columns={columns}
      ...
      <ComposeMessageModal
        ...
        onSent={() => queryClient.invalidateQueries({ queryKey: ['messages-sent', userId] })}
      />
        ...
        {selected && (
          <div className="space-y-4">
            <div className="flex items-center justify-between text-sm text-[#6b7280] border-b border-gray-100 pb-3">
              ...
              <div>
                <span className="font-semibold text-[#111827]">Sent: </span>
